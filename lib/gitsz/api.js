...
class API {
    ...
    async.waterfall(
      [
        ...
        (p, callback) => {
          ...
          const isLegacy = match[1].toLowerCase() === 'sz-v0';

          const hash = new (isLegacy ? gsz.LegacyHash : gsz.Hash)(options);
  ...
  sign(tag, ref, options, callback) {
    ...
    async.waterfall(
      [
        (callback) => {
          const hash = new (options.legacy ? gsz.LegacyHash : gsz.Hash)(options);
